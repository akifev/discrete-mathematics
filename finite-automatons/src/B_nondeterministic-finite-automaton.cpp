...
void strange_func(vector<pair<unordered_set<ll>, unordered_map<char, vector<ll>>>> &dest, ll cur, ll pos) {
...
int main() {
	...
	vector<pair<unordered_set<ll>, unordered_map<char, vector<ll>>>> dest(n);
	for (ll i = 0; i < m; i++) {
		ll a, b;
		char c;
		cin >> a >> b >> c;
		...
		if (dest[a].second.count(c) == 0)
			...
			dest[a].second.at(c).push_back(b);
	}
